...
public class CardRedeemController {
	...
	void initialize() {
		 ...
		 * "SELECT `Số thẻ`, `type`, `ID khách hàng`, `amount`, `lastuseddate`, `registerdate`, `Ngày hết hạn`, `pin`, `description` FROM `card` WHERE `Số thẻ`='"
		 * +cardno+"';";
		 * 
		 ...
		 * if(rs.next()) {
		 * 
		 * carduser.setCardno(rs.getString("Số thẻ"));
		 * carduser.setCustomrid(rs.getString("ID khách hàng"));
		 ...
		 * carduser.setExpireddate(rs.getString("Ngày hết hạn"));
